...
ax = fig.add_axes([0.05, 0.08, 0.93, 0.88])
# ax.set_title(' \n Berry curvature in $(xy)$ plane, A=0, h=2\n at $k_z = 0$',
# size=25)
ax.set_xlabel('$k_x$', size=25)
ax.set_ylabel('$k_y$', size=25, rotation=0)
...
cmapFz = colors.LinearSegmentedColormap.from_list(
    "", ["royalblue", "white", (0.3, 0.5, 0.3)])  # "mediumseagreen" "olivedrab" color
Fz = ax.imshow(TrFz, extent=[-pi, pi, -pi, pi], cmap=cmapFz)
# Fz = ax.imshow(TrFz, extent=[0, 2 * pi, 0, 2 * pi], cmap='BrBG')
cbar = fig.colorbar(Q.lines, pad=0.05)
cbar.ax.set_xlabel('    $(F_x^2+F_y^2)^{1/2}$', size=25, rotation=0)
cbar.ax.tick_params(labelsize=20, width=2)
# cbar.set_clim(0, 2)
# cbar.set_ticks([0.5, 1, 1.5, 2])
cbar.outline.set_linewidth(2)
cbar2 = fig.colorbar(Fz, pad=0.06)
cbar2.ax.set_xlabel('$F_z$', size=25)
