...
for nkx in range(0, Nx - 1):
    ...
    for nky in range(0, Ny - 1):
        kky = ky[nky]
        U1 = np.dot(np.conj(uOcc[nkx, nky, nkz, :]),
                    uOcc[nkx + 1, nky, nkz, :])
        U2 = np.dot(np.conj(uOcc[nkx + 1, nky, nkz, :]),
                    uOcc[nkx + 1, nky + 1, nkz, :])
        U3 = np.dot(np.conj(uOcc[nkx + 1, nky + 1, nkz, :]),
                    uOcc[nkx, nky + 1, nkz, :])
        U4 = np.dot(np.conj(uOcc[nkx, nky + 1, nkz, :]),
                    uOcc[nkx, nky, nkz, :])
        TrF[nkx, nky] = - ((cmath.log(U1 * U2 * U3 * U4)).imag
                           * Nx * Ny / (2*pi)**2)
